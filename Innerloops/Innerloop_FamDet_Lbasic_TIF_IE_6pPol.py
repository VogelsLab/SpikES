...
class Innerloop_FamDet_Lbasic_TIF_IE_6pPol(Innerloop):
    """
    Information about Innerloop_FamDet_Lbasic_TIF_IE_6pPol:
    ...
    def score(self, A):
        ...
        for rule_num in range(self.Nr):
            rule_str = " --rule a" + str(np.exp(A[rule_num][0])) + "a" + str(np.exp(A[rule_num][1]))
            rule_str += "a" + str(A[rule_num][2]) +  "a" + str(A[rule_num][3]) + "a" + str(A[rule_num][4]) +\
             "a" + str(A[rule_num][5]) + "a"
        
            for dataset_num in range(self.Nd):
                cl_str = self.auryn_sim_dir + "sim_innerloop_FamDet_Lbasic_TIF_IE_6pPol --ID " + str(id) +\
                    " --NE " + str(self.NE) +\
                    " --NI " + str(self.NI) +\
                    " --wee " + str(self.wee) +\
                    " --wei " + str(self.wei) +\
                    ...
                    " --eta " + str(self.eta) +\
                    rule_str +\
                    " --wmax " + str(np.exp(random.uniform(np.log(self.wmax_low), np.log(self.wmax_high)))) +\
