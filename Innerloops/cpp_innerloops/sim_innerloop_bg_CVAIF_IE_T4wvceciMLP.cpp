...
int main(int ac, char* av[]) 
{
    ...
    try {
        ...
        desc.add_options()
			...
			("rule", po::value< std::string >(), "plasticity rule, to enter as a string with separator a, one a at the beginning")
	...
	int expected_rule_size = 5 + nh1*6 + nh2*nh1 + 2*(nh2+1); //eta, 4 timescales, first layer, second layer, and last layer with bias (twice, once for on pre update, once for on post)
	//////////////////////////////////////////////////////////////////////////////////////////////////////////
	// std::cout << "inside auryn, putative size of rule " << expected_rule_size << std::endl; 
	//////////////////////////////////////////////////////////////////////////////////////////////////////////
	rule.resize(expected_rule_size); 
	rule = parse_input_plasticity(rule_str, expected_rule_size);
	...
	T4wvceciMLPConnection* con_ie = new T4wvceciMLPConnection(neurons_i, neurons_e, wie, sparseness, rule, nh1, nh2, wmax, GABA, "T4wvceciMLPConnection");
