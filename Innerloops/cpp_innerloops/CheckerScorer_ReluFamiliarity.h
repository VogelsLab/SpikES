#ifndef CHECKERSCORER_RELUFAMILIARITY_H_
#define CHECKERSCORER_RELUFAMILIARITY_H_
 ...
 * It implements both a rate checker and computes online the loss for the FamiliarityNet Innerloop in synapseek.
 * It computes the firing rate of the readout unit for the loss calculation
 * The firing rates are computed as a moving average with time constant tau.
 * 
 * 
...
class CheckerScorer_ReluFamiliarity : public Checker
{
private:
	...
	int itb;
	void init(AurynFloat min, AurynFloat max, AurynFloat tau_pop);
...
public:
	 ...
	 */
	CheckerScorer_ReluFamiliarity(SpikingGroup* source, AurynFloat min, AurynFloat max, AurynFloat tau_pop, AurynFloat rate_cut_fam, AurynFloat rate_cut_new);
	virtual ~CheckerScorer_ReluFamiliarity();
...
#endif /*CHECKERSCORER_RELUFAMILIARITY_H_*/