...
int main(int ac, char* av[]) 
{
	...
	std::vector<std::string> rule_strEE;
	std::vector<float> ruleIE(6);
	std::vector<std::string> rule_strIE;
	float wmaxee;
	float wmaxie;
	float sparseness;
	float l_train;
    ...
    try {
        ...
        desc.add_options()
			...
			("ruleEE", po::value< std::vector<std::string> >(), "plasticity rule, to enter as a string with separator a, one a at the beginning")
			("ruleIE", po::value< std::vector<std::string> >(), "plasticity rule, to enter as a string with separator a, one a at the beginning")
			("wmaxee", po::value<float>(), "max exc weight")
			("wmaxie", po::value<float>(), "max inh weight")
		...
		if (vm.count("ruleEE")) {rule_strEE = vm["ruleEE"].as< std::vector<std::string> >();}
		if (vm.count("ruleIE")) {rule_strIE = vm["ruleIE"].as< std::vector<std::string> >();}
		if (vm.count("wmaxee")) {wmaxee = vm["wmaxee"].as<float>();}
		if (vm.count("wmaxie")) {wmaxie = vm["wmaxie"].as<float>();}
	...
	while ((pos = s.find(delimiter)) != std::string::npos) { // parse the rest of the expression
		...
		ruleEE[ct] = boost::lexical_cast<float>(token);
		s.erase(0, pos + delimiter.length());
		ct ++;
	}

	float tau_preEE = ruleEE[0];
	float tau_postEE = ruleEE[1];
	float alphaEE = ruleEE[2];
	float betaEE = ruleEE[3];
	float gammaEE = ruleEE[4];
	float kappaEE = ruleEE[5];
	...
	while ((pos = s.find(delimiter)) != std::string::npos) { // parse the rest of the expression
		token = s.substr(0, pos);
		ruleIE[ct] = boost::lexical_cast<float>(token);
		s.erase(0, pos + delimiter.length());
		ct ++;
	}

	float tau_preIE = ruleIE[0];
	float tau_postIE = ruleIE[1];
	float alphaIE = ruleIE[2];
	float betaIE = ruleIE[3];
	float gammaIE = ruleIE[4];
	float kappaIE = ruleIE[5];
	...
	SixParamConnection* con_ee = new SixParamConnection(neurons_e, neurons_e, wee, sparseness, eta, alphaEE, betaEE, gammaEE, kappaEE, tau_preEE, tau_postEE, wmaxee, GLUT);
	SparseConnection* con_ei = new SparseConnection(neurons_e, neurons_i, wei, sparseness, GLUT);
	SixParamConnection* con_ie = new SixParamConnection(neurons_i, neurons_e, wie, sparseness, eta, alphaIE, betaIE, gammaIE, kappaIE, tau_preIE, tau_postIE, wmaxie, GABA);
	// SparseConnection* con_ie = new SparseConnection(neurons_i, neurons_e, wie, sparseness, GABA);
	...
	WeightMonitor * wmonee = new WeightMonitor(con_ee, sys->fn("con_ee","syn"), 0.01);
	wmonee->add_equally_spaced(100);
	WeightMonitor * wmonie = new WeightMonitor(con_ie, sys->fn("con_ie","syn"), 0.01);
	wmonie->add_equally_spaced(100);
	///////////////////////////////////////////////////////
	...
	bool familiar_first = true; ////////////////////////////////////////////TO PUT BACK TO FALSE
	///////////////////////////////// DEBUG ///////////////////////////////////
