...
class Innerloop_bg_CVAIF_IE_T4wvceciMLP(Innerloop):
    """
    Information about Innerloop_bg_CVAIF_IE_T4wvceciMLP:
    ...
    def __init__(self, inner_loop_params):
        ...
        self.rule_constant_part = inner_loop_params['rule_constant_part']       #Part of MLP that is not learned
        self.nh1 = inner_loop_params['nh1']
        self.nh2 = inner_loop_params['nh2']
        self.n_params_MLP = 5 + self.nh1*6 + self.nh2*self.nh1 + 2*(self.nh2+1) #eta, 4 timescales, first layer, second layer, and last layer with bias (twice, once for on pre update, once for on post)
    ...
    def score(self, A):
        ...
        for rule_num in range(self.Nr):
            ...
            rule_str = make_rule_str(A[rule_num], self.rule_constant_part)
            ...
            for dataset_num in range(self.Nd):
                ...
                cl_str = self.auryn_sim_dir + "sim_innerloop_bg_CVAIF_IE_T4wvceciMLP --ID " + str(id) +\
                    " --NE " + str(self.NE) +\
                    " --NI " + str(self.NI) +\
                    " --wee " + str(self.wee) +\
                    " --wei " + str(self.wei) +\
    ...
    def __str__(self):
        return("Above: parameters from Innerloop_bg_CVAIF_IE_T4wvceciMLP: " + str(print(vars(self))))
...
def convert_wmats_to_vector_for_auryn(eta=None,
                                      tau_pre1=None,
                                      tau_pre2=None,
                                      tau_post1=None,
                                      tau_post2=None,
                                      ...
                                      Wpost=None):
    """
    Takes matrices from a pytorch implementation of the T4wvceciMLP 
    and converts then to a vector that can be sent to auryn which will be interpreted properly
    typical pytorch objects and dimensions expected:
    args:
        Wh1: tensor size [nh1 (output), input size]: "net.l1.weight" is expected
        Wh2: tensor size [nh2 (output), nh1]: "net.l2.weight" is expected
        Wpre: tensor size [1 (output), nh2 + 1]: concatenate weight and bias: e.g. 
                            "torch.concat((net.l3.weight, net.l3.bias.reshape(-1,1)), dim=1)" is expected
        Wpost: tensor size [1 (output), nh2 + 1]: concatenate weight and bias: e.g. 
                            "torch.concat((net.l3.weight, net.l3.bias.reshape(-1,1)), dim=1)" is expected
    returns:
        1D numpy array [eta, taus..., Wh1, Wh2, Wpre, Wpost]
    """
    res = [eta, tau_pre1, tau_pre2, tau_post1, tau_post2]
    for i in Wh1.flatten().detach().numpy():
        res.append(i)
    for i in Wh2.flatten().detach().numpy():
        res.append(i)
    for i in Wpre.flatten().detach().numpy():
        res.append(i)
    for i in Wpost.flatten().detach().numpy():
        res.append(i)
    return np.array(res)
